...
__all__ = ['AdaptiveKLController', 'FixedKLController', 'PPOModel']
...
class PPOModel:
    ...
    def __init__(self, model, ref_model, tokenizer, **ppo_params):
        ...
        Args:
            model (torch.model): Hugging Face transformer GPT2 model with value head：GPT2HeadWithValueModel
        ...
        # 先定义两个 GPT 模型，一个用于强化，一个用于参考（因为我们通常也不希望强化的模型完全朝着更高的奖励去优化，
        # 所以可以约束一下强化的模型和原始模型仍然具有一定的“相似性”），参考模型不用于优化，所以设为 eval 模式即可
    ...
    def batched_forward_pass(self, queries, responses):
        ...
        for i in range(int(bs / fbs)):
            ...
            input_ids = self.data_collator([torch.cat([q, r]) for q, r in zip(query_batch, response_batch)])[
                "input_ids"]
            with torch.no_grad():
                logits, _, v = self.model(input_ids)  # logits -> (batch, max_output_len, vocab_size);
                                                      # v -> (batch, max_ouput_len)
